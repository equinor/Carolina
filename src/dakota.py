...
class DakotaBase(object):
    ...
    def __init__(self, dakota_input):
        ...
        if dakota_input is None:
            raise RuntimeError(
                "The problem definition is required - None value received"
            )
    ...
    def run_dakota(
        ...
        # Run dakota
        run_dakota(
            infile, stdout, stderr, restart=restart, throw_on_error=throw_on_error
        )
...
class DakotaInput(object):
    ...
    def __init__(self, **kwargs):
        ...
        for key in kwargs:
            if key == "interface":
                raise RuntimeError(
                    "It is not allowed to change the interface. "
                ...
                if section == "interface":
                    ...
                    for line in getattr(self, section):
                        if "analysis_components" in line:
                            raise RuntimeError(
                                "The analysis_components is only allowed to contain "
...
def run_dakota(infile, stdout=None, stderr=None, restart=0, throw_on_error=True):
    ...
    err = 0
    exc = _ExcInfo()
    err = carolina.run_dakota(infile, stdout, stderr, exc, restart, throw_on_error)
    ...
    if err:
        if exc.type is None:
            ...
            reraise(exc.type, exc.type(exc.value), exc.traceback)
...
def dakota_callback(**kwargs):
    ...
    try:
        ...
        msg = "dakota_callback (%s): identifier %s not found in user data" % (
            os.getpid(),
            acs[0],
        )
        logging.error(msg)
